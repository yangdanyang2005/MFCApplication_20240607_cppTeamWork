...
void SignInDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK3, isAutoLogIn);
}
...
BEGIN_MESSAGE_MAP(SignInDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK3, &SignInDlg::OnBnClickedCheckAutoLogIn)
END_MESSAGE_MAP()
...
BOOL SignInDlg::OnInitDialog()
{
	...
	CButton* pCheckBox3 = (CButton*)GetDlgItem(IDC_CHECK3); // 获取是否自动登录的复选框控件的指针

	pCheckBox2->SetCheck(BST_UNCHECKED);//默认不记住密码
	pCheckBox3->SetCheck(BST_UNCHECKED);//默认不自动登录
	pCheckBox3->EnableWindow(FALSE);//如果没有勾选IDC_CHECK2，那么IDC_CHECK3将被禁用

	//如果用户上一次已经选择了“自动登录”，那么应该勾选“自动登录”复选框
	if (isAutoLogInGlobal == 1)
	{
		pCheckBox3->SetCheck(BST_CHECKED);
	}
	...
	else
	{
		...
		pCheckBox3->EnableWindow(TRUE);//由于已经有密码文件，默认勾选IDC_CHECK2，所以IDC_CHECK3将被启用
	}
...
void SignInDlg::OnBnClickedPasswordstyle()
{
	...
	if (isRememberPassword)
	{
		...
		// 保存到文件
		ofstream outFile("user_data/user_credentials.dat", ios::out | ios::trunc | ios::binary);
		if (outFile.is_open())
		{
	...
	{
		remove("user_data/user_credentials.dat");//删除密码文件
	}
...
void SignInDlg::OnBnClickedRememberPasswordStyle()
{
	...
	if (state == BST_CHECKED)
	{
		...
		else
		{
			...
			//禁用IDC_CHECK3
			pCheckBox3->SetCheck(BST_UNCHECKED);//设置不自动登录
			pCheckBox3->EnableWindow(FALSE);
		}
	}
	return;
}
...
void SignInDlg::OnBnClickedCheckAutoLogIn()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE); // 更新控件对应变量的值
	CButton* pCheckBox3 = (CButton*)GetDlgItem(IDC_CHECK3); // 获取是否记住密码的复选框控件的指针
	int state = pCheckBox3->GetCheck();
	if (state == BST_CHECKED)
	{
		ofstream outFile("user_data/user_auto_login.dat", ios::out | ios::trunc | ios::binary);
		if (outFile.is_open())
		{
			outFile << "try";
			outFile.close();
			string usernameRememberMsg1 = "您已设置在每次启动软件时自动登录账户：";
			string usernameRememberMsg2 = CT2A(m_username.GetBuffer()) + "\n您可以通过回到本对话框，取消勾选“下次自动登录”以关闭此功能。";
			string usernameRememberMsg=usernameRememberMsg1+usernameRememberMsg2;
			MessageBox(usernameRememberMsg.c_str(), _T("自动登录账户提示"), MB_OK);

		}
		else
		{
			AfxMessageBox(_T("设置自动登录失败。可能是软件相关数据文件损坏导致。"));
		}
	}
	else
	{
