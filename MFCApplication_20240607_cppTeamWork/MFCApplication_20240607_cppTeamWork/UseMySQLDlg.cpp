...
BEGIN_MESSAGE_MAP(UseMySQLDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON10, &UseMySQLDlg::OnBnClickedButtonMySQLToCsv)
END_MESSAGE_MAP()
...
void UseMySQLDlg::OnBnClickedButtonMySQLToCsv()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
	CFrameWnd* pMainFrame = (CFrameWnd*)AfxGetMainWnd();
	...
	IsOverwriteFromMySQLToCsvDlg dlg;
	dlg.DoModal();

	switch (dlg.isOverwriteFileFlag)
	{
	case 1: // 覆盖写入
	{
		writePointsToCSV(pDoc->arrSQL, filePath);
		break;
	}
	case 2: // 另存为
	{

		CString baseName = _T("gcp");
		CString extension = _T(".csv");
		CString csvFilePathSaveAs;
		CString filePureName;
		filePureName.Format(_T("%s%d%s"), baseName, nSaveCsvCount, extension);

		//通过对话框获取文件路径
		CFileDialog dlg1(FALSE, _T("csv"), filePureName, OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY, _T("CSV Files (*.csv)|*.csv|All Files (*.*)|*.*||"));
		if (dlg1.DoModal() == IDOK)
		{

			csvFilePathSaveAs = dlg1.GetPathName();

			// 打开文件
			CStdioFile csvFile;
			if (csvFile.Open(csvFilePathSaveAs, CFile::modeCreate | CFile::modeWrite | CFile::typeText))
			{
				string csvFilePathSaveAsStr = CT2A(csvFilePathSaveAs);

				// 关闭文件
				csvFile.Close();

				// 写入CSV内容
				writePointsToCSV(pDoc->arrSQL, csvFilePathSaveAsStr);

				MessageBox(_T("CSV文件已成功创建！"));
				nSaveCsvCount++;
