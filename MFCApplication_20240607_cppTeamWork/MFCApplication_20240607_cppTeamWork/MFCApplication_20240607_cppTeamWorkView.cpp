...
CMFCApplication20240607cppTeamWorkView::CMFCApplication20240607cppTeamWorkView() noexcept
{
	...
	//用于自动登录
	{
		...
		if (isLoginSetLastTime == "try")
		{
				...
				)
			{
				...
				isAutoLogInGlobal=true;
			}
			else
			{
				isSignIn = false;
				isAutoLogInGlobal = false;
			}
		}
	...
	//用于自动激活VIP
	{
		...
		if (isAutoVIPLastTime == "try")
		{
			...
			{
				isBuyVIP = true;
				isCheckAutoActivateVIPGlobal = true;
			}
			else
			{
				isBuyVIP = false;
				isCheckAutoActivateVIPGlobal = false;
			}
		}
		inFileAutoVIP.close();
	}
...
void CMFCApplication20240607cppTeamWorkView::OnDrawDEM()
{
	...
	// MATLAB 可执行文件的路径
	const std::string matlabPath = R"(D:\Program Files\MATLAB\R2024b\bin\matlab.exe)";
	// 脚本的相对路径
