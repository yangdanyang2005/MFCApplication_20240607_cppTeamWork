...
void BuyVIPDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK3, isCheckAutoActivateVIP);
}
...
BEGIN_MESSAGE_MAP(BuyVIPDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK3, &BuyVIPDlg::OnBnClickedCheckAutoActivateVIP)
END_MESSAGE_MAP()
...
BOOL BuyVIPDlg::OnInitDialog()
{
	...
	CButton* pCheckBox3 = (CButton*)GetDlgItem(IDC_CHECK3); // 获取是否自动激活VIP的复选框控件的指针
	...
	//如果用户上一次已经选择了“自动激活VIP”，那么应该勾选“自动激活VIP”复选框
	if (isCheckAutoActivateVIPGlobal)
	{
		pCheckBox3->SetCheck(BST_CHECKED);
	}

	ifstream inFile("user_data/user_vipkey.dat", ios::in | ios::binary);
	if (inFile.is_open())
	{
		pCheckBox1->EnableWindow(FALSE);//禁用是否显示密钥的复选框
		pCheckBox2->SetCheck(BST_CHECKED);//如果已经有密钥文件，则默认选中记住密码的复选框
		pCheckBox3->EnableWindow(TRUE);//由于已经有密钥文件，默认勾选IDC_CHECK2，所以IDC_CHECK3将被启用
	}
...
void BuyVIPDlg::OnBnClickedVIPkeystyle()
{
	...
	if (isRememberVIPKey)
	{
		...
		// 保存到文件
		ofstream outFile("user_data/user_vipkey.dat", ios::out | ios::trunc | ios::binary);
		if (outFile.is_open())
		{
...

{
	...
	if (state == BST_CHECKED)
	{
		...
		{
			remove("user_data/uesr_vipkey.dat");//删除密钥文件
			pCheckBox2->SetCheck(BST_UNCHECKED); // 取消选中复选框


			//禁用IDC_CHECK3
			pCheckBox3->SetCheck(BST_UNCHECKED);//设置不自动激活VIP
...
void BuyVIPDlg::OnBnClickedCheckAutoActivateVIP()
{
	// TODO: 在此添加控件通知处理程序代码
		// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE); // 更新控件对应变量的值
	...
	if (state == BST_CHECKED)
	{
		ofstream outFile("user_data/user_auto_vip.dat", ios::out | ios::trunc | ios::binary);
		if (outFile.is_open())
		{
			outFile << "try";
			outFile.close();
			MessageBox(_T("您已设置在每次启动软件时自动激活VIP。\n您可以通过回到本对话框，取消勾选“自动激活VIP”以关闭此功能。"), _T("VIP自动激活提示"), MB_OK);
		}
		else
		{
			AfxMessageBox(_T("设置自动激活VIP失败。可能是软件相关数据文件损坏导致。"));
		}
	}
	else
	{
